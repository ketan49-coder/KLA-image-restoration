...
$$\mathcal{L}_{\text{Tri-Fidelity}} = \lambda_1 \mathcal{L}_{\text{Charbonnier}} + \lambda_2 \mathcal{L}_{\text{MS-SSIM}} + \lambda_3 \mathcal{L}_{\text{FFL}}$$
...
### 3. Adaptive High-Frequency Edge Restoration ($\mathcal{L}_{\text{FFL}}$)
Focal Frequency Loss (FFL) computes the 2D Real Fast Fourier Transform (FFT) of both the prediction and the ground truth. To handle SEM sensor noise without overreacting to frequency outliers, we apply a Charbonnier penalty to the magnitude differences: $\sqrt{\Delta\mathcal{F}^2 + \epsilon^2}$. 
Crucially, we employ **dynamic focal weighting**—the loss is scaled by the *current detached error* raised to a power $\gamma$. As training progresses, this forces the network to automatically hunt down whichever frequency bands (e.g., sharp edges) it is still struggling to reconstruct, rather than statically weighting against the ground truth.

**Conclusion:** By fusing these three distinct paradigms (Robust Intensity, Structure, and Adaptive Frequency), we achieved an explosive convergence rate, hitting **>27 dB PSNR in just 14 epochs** during our ablation trials (Run 13_Hybrid), and have now optimized it even further for the final evaluation.
